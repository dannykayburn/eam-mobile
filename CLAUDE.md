...
| **Six cross-device rules — APPLIED 2026-09-21** | §31.5 | All six are done and verified (every screen loads, 10/10 tests). **Every one had been invisible on the device it was authored on**, which is the argument for auditing against the rule, not a device. What landed: pinch-zoom unblocked on every screen; **354 `font-size` declarations px→rem** (exact /16, so identical at a 16px root) with **6 deliberate px exceptions** — glyphs inside fixed-size circles, which scale with their control not with reading text; hit areas grown to 48px by transparent `::after`, **painted sizes untouched**; a solid `background` before `.bottom-nav`s `color-mix()`; all storage via **`lsGet()`/`lsSet()`/`lsRemove()`** (never call `localStorage` directly again — it throws in iOS Private Browsing and the symptom is a control *silently doing nothing*); and both fonts self-hosted, so the app now has **zero runtime network dependencies**. Residue in §20: the top-nav cluster cannot reach 48×48 without a §4.2 spacing call, and `mockups/`/`old versions/` still block zoom on purpose. |
| **Native-app feel — and the ZOOM TRAP** | §31.6 | **Two things are called zoom and conflating them caused a wrong fix once.** **Pinch**-zoom is never blocked (it is an accessibility affordance, the app never *needs* it) — and blocking it buys no native feel anyway, because **iOS has ignored `user-scalable=no` for pinch since iOS 10**, so the old tag only restricted Android. The real jank is **iOS force-zooming the page when a text input under 16px takes focus, and not zooming back**. Fix is to **remove the trigger: every text input is `1rem`** (14 rules raised from 13/14/15px — accepted visible cost; iOS system field text is 17pt, so this is *closer* to native). **Never cap viewport scale to fix a field.** What actually delivers app feel, all now in `eam-shared.css`: tap-highlight transparent (the grey flash was the biggest web tell), `overscroll-behavior:none`, `user-select`/`touch-callout` **scoped to chrome so record values stay copyable**, and `touch-action:manipulation` (**not `none`** — that breaks scrolling). Plus a **PWA manifest** + apple meta + generated 180/192/512 icons: Add to Home Screen launches fullscreen, no browser UI. **Install gotchas, both found on device:** the **manifest lives at the REPO ROOT** so `index.html` is inside its `scope` — it was one directory down and the root could not launch standalone at all; and the tags must be on `index.html`/`screens.html` too, not just the screens, since the root URL is what people actually add. **An existing home-screen icon NEVER upgrades itself** — iOS snapshots the config at add time, so changes need a delete-and-re-add. iOS <16.4 also leaves the installed window on cross-file navigation — §20. |
| **Bottom breathing room — `--shell-pad-bottom`** | §31.6 | Added 2026-09-21 from device feedback once standalone install worked: **in a tab, Safari's toolbar sat below the app and gave separation; installed, the app's bottom edge IS the device edge**, so the chrome reads as smooshed. **Do NOT pad `body` or `.app` to fix this** — body padding lifts the bottom nav off the edge and breaks §4.2 ("anchored, not floating... flush to the bottom edge"), and `.app` padding does nothing at all, since an abs-positioned child's containing block is the **padding box** so `bottom:0` ignores it. The room goes **inside** the chrome: both bars stay `bottom:0` and grow downward (bar 72→84 with `padding-bottom`, nav 80→92), `--bar-reserve` tracks the taller bar or content clips behind it, and `.content` gains the same pad for screens with no bottom chrome. The token carries `env(safe-area-inset-bottom, 0px)` like `--bar-reserve` does, so it is already right if `viewport-fit:cover` is ever turned on. |
| **Scroll-collapse must not destroy its own trigger** | §31.6 | Fixed 2026-09-21 from a device report (Home "jumpy... pulls me right back up"). §5.3's collapse fires past `scrollTop>40` and restores under `10`, and it collapses by **reflow** (`max-height` — a transform would not grow the scroll area). So on a **marginally scrollable** screen it oscillates: collapse → range gone → browser clamps scrollTop to 0 → expand → scrollable again → repeat. **Guard: only collapse when `(scrollHeight-clientHeight) - chrome.offsetHeight > 40`.** `offsetHeight` is valid there because the branch only runs while expanded, and the chrome may sit *outside* the scroll container (Home's does), where collapsing grows `clientHeight` instead — same cost, one check covers both. Consequence is intended: a barely-scrolling screen never collapses its chrome. **`#recHeader` guarded identically** — same listener, same reflow, a sparse record would reproduce it. **Latent all along**; the §31.6 bottom padding only moved Home into range, so don't read the symptom as the cause. |
| **Assets carry a `?v=` cache stamp** | §31.6 | `bump-assets.js` stamps `eam-shared.css`/`.js`, `eam-fonts.css` and `data/*.js` across every deployed screen; images are excluded (they never change, and iOS captures the touch icon at install). **Run it on every deploy that touches a stamped asset** — Pages sends `max-age=600` and an installed standalone app has **no reload button**, so a device can silently sit on a stale build, which during a research session means testing the wrong thing. `--check` fails on an unversioned or mixed-version ref, which is what stops a new screen drifting. **The stamp is date+TIME**: date alone meant two same-day deploys produced an identical stamp and busted nothing. Anything that resolves these paths by hand must strip the query — `run-load.js` had to be taught to (browsers ignore it on `file://`, `path.join` does not). |
